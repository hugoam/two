...
static TextScript& create_wren_script(WrenInterpreter& interpreter)
{
	...
	script.m_script =
		"\n"
		"//n = gfx.node(scene, null, vec3.create(0), quat.create(1,0,0,0), vec3.create(1))\n"
		"//gfx.shape(n, Cube.create(), Symbol.create(Colour.White, Colour.None, false, false, SymbolDetail.Medium), 0, null, 0)\n"
		"\n"
		"for (x in 1..11) {\n"
		"    for (y in 1..11) {\n"
		"\n"
		"        var angles = vec3.create(time + x * 0.21, 0, time + y * 0.37)\n"
		"        var pos = vec3.create(-15 + x * 3, 0, -15 + y * 3)\n"
		"\n"
		"        var r = ((time + x * 0.21).cos + 1.0) / 2.0\n"
		"        var b = ((time + y * 0.37).sin + 1.0) / 2.0\n"
		"        var g = ((time).cos + 1.0) / 2.0\n"
		"        var color = Colour.create(r, g, b, 1)\n"
		"\n"
		"        var s = Symbol.create(color, Colour.None, false, false, SymbolDetail.Medium)\n"
		"        var n = gfx.node(scene, null, pos, quat.create(angles), vec3.create(1))\n"
		"        gfx.draw(n, Cube.create(), s, 0)\n"
		"\n"
		"    }\n"
		"}\n"
		"\n"
