...
var fxaa_vertex = `$input a_position, a_texcoord0
        $output v_uv0
...
if(init)
{
    var program = app.gfx.programs.create('fxaa');
    program.set_source(two.ShaderType.Vertex, fxaa_vertex);
    program.set_source(two.ShaderType.Fragment, fxaa_fragment);
}
...
if(init)
{
    ...
    for(var i = 0; i < 100; ++i)
    {
        var p = new two.vec3(Math.random() * 500.0 - 250.0, Math.random() * 500.0 - 250.0, Math.random() * 500.0 - 250.0);
