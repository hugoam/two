...
void xx_shadow_point(Shell app, Widget parent, Dockbar dockbar)
{
	...
	if(!once)
	{
		...
		viewer.scene.env.radiance.ambient = 0.2f;
	}
	...
	function light_source = [](Gnode parent, Colour color, var pos, quat rot) -> Gnode
	{
		...
		var outer = two.gfx.shape(node, new two.Sphere(2.0), Symbol::plain(Colour::White), 0U, sphere_material);
		
		Light light = two.gfx.light(node, LightType::Point, true, color, range, 0.6f);// intensity);
		light.range = range;
	...
	Gnode light0 = light_source(scene, two.rgba(0x0088fff), pos0, rot0);
	...
	Gnode light1 = light_source(scene, two.rgba(0xff8888f), pos1, rot1);

	Box box = new two.Cube(new two.vec3(15.0));
	Gnode node = two.gfx.node(scene, {});// , Y3 * 10.0);
	two.gfx.shape(node, box, Symbol::plain(Colour::White), 0U, cube_material);
}
