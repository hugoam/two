...
namespace mud
{
	...
	struct GpuState<MaterialBase>
	{
		...
		{
			u_uv0_scale_offset = bgfx::createUniform("u_material_params_0", bgfx::UniformType::Vec4, array_size);
			u_uv1_scale_offset = bgfx::createUniform("u_material_params_1", bgfx::UniformType::Vec4, array_size);
		}
	...
	struct GpuState<MaterialUnshaded>
	{
		void init(uint16_t array_size = 1U)
		{
			u_color = bgfx::createUniform("u_color", bgfx::UniformType::Vec4, array_size);
		}
	...
	struct GpuState<MaterialFresnel>
	{
		void init(uint16_t array_size = 1U)
		{
			u_fresnel_params = bgfx::createUniform("u_fresnel_params", bgfx::UniformType::Vec4, array_size);
			u_fresnel_value = bgfx::createUniform("u_fresnel_value", bgfx::UniformType::Vec4, array_size);
		}
	...
	struct GpuState<MaterialPbr>
	{
		void init(uint16_t array_size = 1U)
		{
			u_albedo = bgfx::createUniform("u_albedo", bgfx::UniformType::Vec4, array_size);
			u_pbr_params_0 = bgfx::createUniform("u_pbr_params_0", bgfx::UniformType::Vec4, array_size);
			u_pbr_params_1 = bgfx::createUniform("u_pbr_params_1", bgfx::UniformType::Vec4, array_size);
			u_pbr_channels_0 = bgfx::createUniform("u_pbr_channels_0", bgfx::UniformType::Vec4, array_size);
			u_emissive = bgfx::createUniform("u_emissive", bgfx::UniformType::Vec4, array_size);
		}
	...
	struct GpuState<Material>
	{
		...
		{
			GpuState<MaterialBase>::me.upload(encoder, material.m_base_block);
			if(material.m_unshaded_block.m_enabled)
				GpuState<MaterialUnshaded>::me.upload(encoder, material.m_unshaded_block);
			if(material.m_pbr_block.m_enabled)
				GpuState<MaterialPbr>::me.upload(encoder, material.m_pbr_block);
			//if(material.m_fresnel_block.m_enabled)
			//	GpuState<MaterialFresnel>::me.upload(encoder, material.m_fresnel_block);
		}
	...
	struct GpuState<Material>
	{
		void pack(const Material& material, size_t index, const GpuTexture& buffer, float* dest)
		{
			...
			GpuState<MaterialBase>::me.me.pack(material.m_base_block, offset, buffer, dest);
			GpuState<MaterialUnshaded>::me.me.pack(material.m_unshaded_block, offset, buffer, dest);
			GpuState<MaterialPbr>::me.me.pack(material.m_pbr_block, offset, buffer, dest);
		}
