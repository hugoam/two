...
namespace mud
{
	...
	void PassShadowmap::submit_render_pass(Render& render)
	{
		...
		for(Light* light : render.m_shot->m_lights)
		{
			...
			{
				uvec4 atlas_rect = render.m_shadow_atlas->render_update(render, *light);
				...
				for(int i = 0; i < 6; i++)
				{
					...
					ShadowCubemap& cubemap = render.m_shadow_atlas->light_cubemap(*light, uint16_t(rect_w(atlas_rect)));
			...
			else if(light->m_type == LightType::Spot)
			{
				...
				ShadowRender shadow_render = { render, *light, render.m_shadow_atlas->m_fbo, atlas_rect, projection, light->m_node.transform() };
				shadow_render.cull();
				shadow_render.render(m_block_shadow.m_block_depth, 1.f);
			}
		}
	...
	void BlockShadow::submit_gfx_element(Render& render, Pass& render_pass, DrawElement& element)
	{
		...
		if(render.m_shadow_atlas)
		{
			bgfx::setTexture(uint8_t(TextureSampler::ShadowAtlas), u_shadow.s_shadow_atlas, render.m_shadow_atlas->m_depth, BGFX_TEXTURE_COMPARE_LESS);
			vec2 shadow_atlas_pixel_size = vec2(1.f) / float(render.m_shadow_atlas->m_size);
