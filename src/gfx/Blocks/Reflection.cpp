...
namespace mud
{
	...
	void BlockReflection::submit_gfx_element(Render& render, Pass& render_pass, DrawElement& element)
	{
		...
		if(render.m_reflection_atlas && render.m_reflection_atlas->m_size)
			bgfx::setTexture(uint8_t(TextureSampler::ReflectionProbe), u_uniform.s_atlas, render.m_reflection_atlas->m_color_tex);
	...
	void BlockReflection::upload_reflection_probes(Render& render, array<ReflectionProbe*> probes)
	{
		...
		for(ReflectionProbe* probe : probes)
		{
			...
			probe_array.atlas_rect[probe_count] = { render.m_reflection_atlas->probe_rect(*probe) };
	...
	void BlockReflection::render_reflection_probe(Render& render, ReflectionProbe& probe)
	{
		...
		uvec4 atlas_rect = render.m_reflection_atlas->render_update(render, probe);
