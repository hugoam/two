...
namespace mud
{
		...
		for(const glTFImage& image : gltf.m_images)
		{
			if(image.uri != "")
			{
				if(image.uri.find("data:application/octet-stream;base64") == 0)
				{
					...
					Texture& texture = state.m_gfx_system.load_texture_mem("temp", to_array(data));
					state.m_imported_images.push_back(&texture);
				}
				else
				{
					Texture& texture = state.m_gfx_system.load_texture((string(path) + file + "/").c_str(), image.uri.c_str());
					state.m_imported_images.push_back(&texture);
				}
			}
			else if(image.buffer_view != -1)
			{
				...
				Texture& texture = state.m_gfx_system.load_texture_mem("temp", { gltf.m_binary_buffers[buffer_view.buffer].data() + buffer_view.byte_offset, buffer_view.byte_length });
				state.m_imported_images.push_back(&texture);
			}
		}
	}
	...
	void import_meshes(const glTF& gltf, glTFImport& state)
	{
		...
		for(const glTFMesh& gltf_mesh : gltf.m_meshes)
		{
			...
			{
				state.m_model.m_meshes.emplace_back(state.m_gfx_system, state.m_model.m_name, true);
				...
				shape.pack_vertices(state.m_model_config.m_transform);
	...
	void import_materials(const glTF& gltf, glTFImport& state)
	{
		...
		{
			Material& material = state.m_gfx_system.create_material(gltf_material.name.c_str(), "pbr/pbr");
	...
	void ImporterGltf::import_model(Model& model, const string& path, const string& file, const ModelConfig& config)
	{
		printf("INFO: Gltf - loading model %s, %s\n", path.c_str(), file.c_str());
