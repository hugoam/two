...
def register_type(module, parent, clsname):
    for name in ['std::vector', 'std::list', 'mud::array', 'array'] :
...
class Generator(object):
    ...
    def generate_module(self, module):
        ...
        def register_classes(cursor, file, module, parent):
            for c in cursor.get_children():
                if os.path.normpath(c.location.file.name).find(os.path.normpath(module.path)) == 0 and c.location.file.name.find('Generated') == -1:
                    ...
                    elif (c.kind == clang.cindex.CursorKind.CLASS_DECL or c.kind == clang.cindex.CursorKind.STRUCT_DECL or c.kind == clang.cindex.CursorKind.CLASS_TEMPLATE) and c.is_definition():
        ...
        for name in ('Types.h', 'Types.cpp') :
            self.render_mako(module, name, False)
