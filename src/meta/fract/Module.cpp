...
namespace mud
{
	...
	// mud::Circlifier
	{
		...
		static Class cls = { type<mud::Circlifier>(),
			...
			// methods
			{
				{ type<mud::Circlifier>(), "compute", member_address<vector<mud::Circle>(mud::Circlifier::*)(const mud::Colour&, float)>(&mud::Circlifier::compute), [](Ref object, array<Var> args, Var& result) { val<vector<mud::Circle>>(result) = val<mud::Circlifier>(object).compute(val<mud::Colour>(args[0]), val<float>(args[1])); }, { { "colour", var(mud::Colour()) }, { "scale", var(float(1.f)), Param::Default } }, var(vector<mud::Circle>()) }
			},
			// static members
			{
			}
	...
	// mud::Fract
	{
		...
		static Class cls = { type<mud::Fract>(),
			...
			// methods
			{
				...
				{ type<mud::Fract>(), "render_grid", member_address<void(mud::Fract::*)(mud::uvec2, const mud::Pattern&, mud::uvec2, vector<mud::Image256>&)>(&mud::Fract::render_grid), [](Ref object, array<Var> args, Var& result) { UNUSED(result); val<mud::Fract>(object).render_grid(val<mud::uvec2>(args[0]), val<mud::Pattern>(args[1]), val<mud::uvec2>(args[2]), val<vector<mud::Image256>>(args[3])); }, { { "size", var(mud::uvec2()) }, { "pattern", var(mud::Pattern()) }, { "resolution", var(mud::uvec2()) }, { "output_images", var(vector<mud::Image256>()), Param::Output } }, Var() }
			},
			// static members
			{
			}
