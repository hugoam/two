...
namespace mud
{
    ...
    // mud::ScriptError
    {
        static Meta meta = { type<mud::ScriptError>(), &namspc({ "mud" }), "ScriptError", sizeof(mud::ScriptError), TypeClass::Struct };
        static Class cls = { type<mud::ScriptError>(),
            // bases
            {  },
            {  },
            // constructors
            ...
            // copy constructor
            {
                { type<mud::ScriptError>(), [](Ref ref, Ref other) { new(&val<mud::ScriptError>(ref)) mud::ScriptError(val<mud::ScriptError>(other)); } }
            },
            // members
            {
            },
    ...
    // mud::VisualScript
    {
        ...
        static Class cls = { type<mud::VisualScript>(),
            ...
            // constructors
            {
                { type<mud::VisualScript>(), [](Ref ref, array<Var> args) { new(&val<mud::VisualScript>(ref)) mud::VisualScript( val<const char*>(args[0]), val<mud::Signature>(args[1]) ); }, { { "name", Ref(type<const char*>()), Param::Nullable }, { "signature", Ref(type<mud::Signature>()), Param::Default } } }
            },
            // copy constructor
            {
            },
