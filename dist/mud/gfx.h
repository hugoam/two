...
namespace mud
{
	export_ MUD_GFX_EXPORT func_ mat4 bxidentity();
	export_ MUD_GFX_EXPORT func_ mat4 bxinverse(const mat4& mat);
	export_ MUD_GFX_EXPORT func_ mat4 bxSRT(const vec3& scale, const vec3& rot, const vec3& trans);
	export_ MUD_GFX_EXPORT func_ mat4 bxscale(const vec3& scale);
	// transpose because we are right-handed but bgfx assumes left-handed
	export_ MUD_GFX_EXPORT func_ mat4 bxrotation(const quat& rot);
	//export_ MUD_GFX_EXPORT mat4 bxquat(const quat& rot) { mat4 result; bx::mtxQuat(value_ptr(result), &rot[0]); return result; }
	export_ MUD_GFX_EXPORT func_ mat4 bxtranslation(const vec3& vec);
	export_ MUD_GFX_EXPORT func_ mat4 bxmul(const mat4& lhs, const mat4& rhs);
	export_ MUD_GFX_EXPORT func_ vec4 bxmul(const mat4& mat, const vec4& vec);
	export_ MUD_GFX_EXPORT func_ vec3 bxmul(const mat4& mat, const vec3& vec);
	export_ MUD_GFX_EXPORT func_ vec3 bxmulh(const mat4& mat, const vec3& vec);
	export_ MUD_GFX_EXPORT func_ mat4 bxlookat(const vec3& eye, const vec3& at);
	export_ MUD_GFX_EXPORT func_ void bxlookat(mat4& result, const vec3& eye, const vec3& at, const vec3& up);
	export_ MUD_GFX_EXPORT func_ mat4 bxlookat(const vec3& eye, const vec3& at, const vec3& up);
	export_ MUD_GFX_EXPORT func_ mat4 bxproj(float fov, float aspect, float near, float far, bool oglNdc);
	export_ MUD_GFX_EXPORT func_ mat4 bxortho(const vec4& rect, float near, float far, float offset, bool oglNdc);
	export_ MUD_GFX_EXPORT func_ mat4 bxortho(float left, float right, float bottom, float top, float near, float far, float offset, bool oglNdc);
	export_ MUD_GFX_EXPORT func_ mat4 bxSRT(const vec3& scale, const quat& rot, const vec3& trans);
	export_ MUD_GFX_EXPORT func_ mat4 bxTRS(const vec3& scale, const quat& rot, const vec3& trans);
...
namespace gfx
{
	...
	export_ MUD_GFX_EXPORT func_ Flare& flows(Gnode& parent, const Flow& emitter, uint32_t flags = 0);
